...
class FileTag {
  ...
  async addTags(filePath, tags) {
    ...
    // Normalize tags: trim and lowercase, and filter out empty strings
    const normalize = t => (typeof t === 'string' ? t.trim().toLowerCase() : '');
    const newTags = tags.map(normalize).filter(t => t.length > 0);
    if (newTags.length === 0) {
      throw new Error('No valid tags provided');
    }

    // Add new tags (avoid duplicates) while preserving insertion order
